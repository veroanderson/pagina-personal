...
export default function AdminSeriesPage() {
  const { createSeries, listSeries, removeSeries, updateSeries } = useAdminSeries();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const res = editingSeries
        ? await updateSeries(bodyPayload)
        : await createSeries(bodyPayload);
  ...
  const handleDelete = async (id: number, title: string) => {
    ...
    try {
      const res = await removeSeries(id);
      if (res.ok) {
        loadSeries();
      } else {
